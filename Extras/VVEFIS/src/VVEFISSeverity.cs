...
namespace VVEFIS
{
    ...
    //
    // Since the 2026-08-24 "shared basket" refactor the signal READERS
    // (DangIt/FAR/RealBattery bridges, fuel/engine readings, the Tier scale,
    // mod presence checks) live in MFDExtension's src/Shared/ and are
    // source-linked into this DLL - see the csproj and DangItBridge.cs's
    // header. This file keeps only what is VVEFIS-specific: the tier->color
    // mapping, the per-source precedence, and the fill/border delegates.
    //
