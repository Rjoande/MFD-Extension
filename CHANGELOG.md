...
- **CAS key legend now reads `▲▼: scroll  ○: home`** (was `ΛV`). Its scroll/collapse logic moved into a small engine shared by every text bay this package ships, so future bays (SystemHeat, DynamicBatteryStorage) get the same scrolling/truncation for free. CAS itself renders exactly as before.
- **BMS (RealBattery) now cycles through three pages**: a new vessel-wide EPS summary, then the existing per-vessel telemetry, then the fleet view — press **B** to cycle. Hosted bays can now chain through three or more pages on one button, not just toggle between two.

### Extras/VVHolo (optional; requires VesselView Continued)

- **New: "HOLO"**, a third VesselView color mode — a sci-fi holographic look (cobalt fill, light-blue wireframe) instead of a diagnostic readout. Only WARNING/CAUTION get distinct treatment (fill and wireframe turn red, the outline breathes red↔dark red); everything else stays visually nominal. Own `VVHolo.dll`, deletable independently.
